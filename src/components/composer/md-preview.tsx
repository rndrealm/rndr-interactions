  ...
  return (
    <>
      ...
      >
        ...
        >
          <h3 className="text-xs pb-1 font-medium text-foreground">{asset.file.name}</h3>
          <p className="text-[10px] text-foreground/40">{asset.content?.slice(0, 100)}</p>
        </div>
        {onRemove && (
          <button
            onClick={(e) => {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          ...
          className="max-w-3xl max-h-[80vh] flex flex-col gap-0 p-0 overflow-hidden bg-card! border-border! shadow-lg! data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:slide-in-from-top-4 data-[state=open]:duration-300 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:slide-out-to-top-4 data-[state=closed]:duration-200"
        >
          <div className="flex items-start justify-between px-5 pt-5 pb-3">
            <div>
              <h2 className="text-base font-semibold text-foreground">{asset.file.name}</h2>
          ...
          <div className="px-5 pb-5 flex-1 min-h-0">
            <div className="bg-accent rounded-lg p-4 overflow-y-auto max-h-[60vh]">
              <pre className="whitespace-pre-wrap text-xs leading-relaxed font-mono text-foreground/70">
                {asset.content}
              </pre>
            </div>
          </div>
