  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">{selectedModel?.name ?? "Model"}</p>
        <div>
          <ChevronDown
            className={cn("transition-transform text-foreground/40", { "rotate-180": open })}
          />
        </div>
      </DropdownMenuTrigger>
      <DropdownMenuContent
              ...
              <TooltipTrigger
                render={
                  <DropdownMenuItem
                    ...
                    className="flex items-center justify-between cursor-pointer h-9 py-0 hover:bg-accent focus:bg-accent focus:text-inherit"
                  />
                }
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-foreground/70!">{model.name}</span>
                </div>
                {value === model.id && (
